...
client.on("message", async message => {
	...
	if (command === "wiki" || command === "facebook" || command === "google") {
	...
	if(message.member.hasPermission('ADMINISTRATOR', true)){
		...
		if(command === "level"){
			...
			level["level"].level += 1;
			fs.writeFile("./Commands/JSON/level.json", JSON.stringify(level), (err) => {
		...
		if(command === "addc") {
			...
			fs.writeFile("./Commands/JSON/country.json", JSON.stringify(ctry), (err) => {
			if (err) console.log(err);
			});
		...
		if(command === "test"){
			message.delete().catch(O_o => {});
			test.test(message, Discord, config, music, fs, ms, country, help, espera, regras, rep, move, poll, aviso, avisos, lavisos, client, warns, cont, temporizador, timer, chck, ctry, timex, mmbrs, unregistered, mensagem, user, alliance, lead, man, admin, castigo, autor, usern, usern1, embed, ct, args, command, err, membro);
			return;
		}
